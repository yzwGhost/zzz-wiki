...
#### Scenario: Failed subtask retry is requested
- **WHEN** the renderer requests a retry for a failed sync subtask
- **THEN** preload forwards a dedicated retry request through Electron IPC
- **AND** Electron returns a structured payload that separates original failure context from retry execution result
